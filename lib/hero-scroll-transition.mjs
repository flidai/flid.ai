...
export function shouldHeroOccludeStory(storyStickyTop, reducedMotion = false) {
  if (reducedMotion) return false;
  const top = Number(storyStickyTop);
  return Number.isFinite(top) && top > 0;
}

export function createMotionSequenceState(
  scrollOffset = 0,
  introScrollRange = 1,
  storyScrollRange = 1,
) {
  const offset = Math.max(0, Number(scrollOffset) || 0);
  const introRange = Math.max(1, Number(introScrollRange) || 1);
  const storyRange = Math.max(1, Number(storyScrollRange) || 1);
  const introProgress = clamp(offset / introRange);
  const storyProgress = clamp((offset - introRange) / storyRange);

  return {
    introProgress: round(introProgress),
    storyProgress: round(storyProgress),
    storyActive: offset >= introRange,
  };
}
