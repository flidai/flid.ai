...
export const depthVideoStorySettings = Object.freeze({
  sources: Object.freeze([`${sourceRoot}/depth-story.mp4`]),
  densityScale: 1.55,
...
float sourceContentMask(vec2 uv) {
  float watermarkX = smoothstep(0.60, 0.64, uv.x);
  float watermarkY = 1.0 - smoothstep(0.11, 0.14, uv.y);
  return 1.0 - watermarkX * watermarkY;
}

float sourceValue(sampler2D source, vec2 uv) {
  float value = luma(texture(source, uv).rgb) * sourceContentMask(uv);
...
void main() {
  ...
  float entrance = smoothstep(0.0, 1.0, clamp(uEntrance, 0.0, 1.0));
  vec2 imageWorld = aPosition * uPlaneScale;
  float contourAngle = hash11(aSeed * 73.0 + 2.0) * 6.28318530718;
  float contourRadius = sqrt(hash11(aSeed * 131.0 + 5.0));
  float contourBand = floor(contourRadius * 22.0) / 22.0;
  contourBand += (hash11(aSeed * 193.0 + 7.0) - 0.5) * 0.018;
  vec2 contourWorld = vec2(cos(contourAngle), sin(contourAngle)) *
    contourBand * uPlaneScale * vec2(1.18, 0.92);
  vec2 world = mix(contourWorld, imageWorld, entrance);

  float depth = ((value - 0.5) * 0.92 + edge * 0.075) * entrance;
  ...
  vAlpha = imageAlpha * particleOccupancy * entrance;
}
`;
...
export class DepthVideoStory {
  constructor(
    ...
    this.entrance = 1;
    this.particleScale = clamp(particleScale, 0.25, 1);
    this.records = [];
    this.vertexCount = 0;
  ...
  setEntrance(value) {
    this.entrance = clamp(value);
  }

  targetForWidth(width) {
    return Math.round(depthParticleTarget(
      width,
  ...
  render(time = performance.now()) {
    ...
    gl.uniform1f(this.uniforms.uEntrance, this.entrance);
